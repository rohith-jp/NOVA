...
class AuditLogChain:
    ...
    def add_entry(
        self, user_id: str, task_id: str, action_type: str, metadata: Dict[str, Any]
    ) -> AuditLogEntry:
        ...
        for i, entry in enumerate(self.entries):
            ...
            if i == 0:
                if entry.prev_hash != self.GENESIS_HASH:
                    return (
                        False,
                        f"Broken chain: Genesis entry {entry.entry_id} has invalid prev_hash.",
                        i,
                    )
            else:
                ...
                if entry.prev_hash != prev_entry.curr_hash:
                    return (
                        False,
                        f"Broken chain link detected between {prev_entry.entry_id} and {entry.entry_id}.",
                        i,
                    )
